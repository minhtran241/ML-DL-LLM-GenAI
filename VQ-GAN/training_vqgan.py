    ...
    def configure_optimizers(self, args):
        ...
        opt_vq = torch.optim.Adam(
            ...
            betas=(
                args.beta1,
                args.beta2,
            ),  # The exponential decay rate for the first and second moment estimates
        )
        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    decoded_imgs, min_encoding_indices, loss = self.vqgan(imgs)
                    ...
                    lambda_ = self.vqgan.calculate_lambda(perceptual_rec_loss, g_loss)
                    vq_loss = (
                        perceptual_rec_loss + loss + disc_factor * lambda_ * g_loss
                    )
                    ...
                    if i % 10 == 0:
                        with torch.inference_mode():
                            real_fake_images = torch.cat(
                                (imgs[:4], decoded_imgs.add(1).mul(0.5)[:4])
                            )
                            vutils.save_image(
                                real_fake_images,
...
if __name__ == "__main__":
    ...
    args.dataset_path = r"./data/flowers"
